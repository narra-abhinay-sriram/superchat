...
export default function Superchat_translate() {
  ...
  return (
    <main className={`w-full   sm:w-auto    h-screen relative sm:ml-[230px] ${darkmode?"bg-[#2C2C2C]" : "bg-white"}` } >
        <h2 className={`
      flex 
      pt-12 
      mb-4 
    ...
    <div className="relative border-2 rounded-lg mr-3  focus-within:border-transparent">
        <textarea
            ...
            placeholder="Enter text to translate"
             className={ `h-full  w-[500px] p-3 pb-16 rounded-lg border-0  focus:outline-none resize-none ${darkmode? "bg-[#2C2C2C] text-white" : "bg-[white] text-black"}`}/>
     ...
     <div className="relative border-2 rounded-lg  focus-within:border-transparent">
      ...
      <select className={`py-2 pl-1 text-md hover:cursor-pointer rounded-lg m-1 ${darkmode ? "bg-[#2C2C2C] text-white" : "" }`} onChange={(e)=>settarget_lang(e.target.value)}>
